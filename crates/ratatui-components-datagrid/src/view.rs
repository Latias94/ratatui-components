...
impl DataGridView {
    ...
    pub fn set_row_count(&mut self, rows: usize) {
        ...
        if self.options.selection_follows_cursor {
            self.selection = self.cursor.map(Selection::Single).unwrap_or(Selection::None);
        }
        self.state.clamp();
    }
    ...
    fn move_cursor_by(&mut self, drow: i32, dcol: i32, shift: bool) -> bool {
        ...
        let next_row = (cur.row as i64 + drow as i64)
            .clamp(0, self.rows.saturating_sub(1) as i64) as usize;
        ...
        if self.options.multi_select && shift {
            ...
            self.selection = Selection::Rect { start: anchor, end: next };
    ...
    fn render_header(&mut self, area: Rect, buf: &mut Buffer, style: Style, grid_line_style: Style) {
        if area.width == 0 || area.height == 0 {
            return;
        }
        ...
        for col_item in self.col_v.get_virtual_items() {
            ...
            let title = render::slice_by_cols(&col.title, clip_left, rect.width);
            buf.set_stringn(rect.x, rect.y, title, rect.width as usize, style);
