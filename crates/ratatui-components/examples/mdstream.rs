...
mod with_mdstream {
    ...
    use ratatui_components::markdown::streaming::MarkdownStreamView;
    use ratatui_components::markdown::view::MarkdownView;
    use ratatui_components::markdown::view::MarkdownViewOptions;
    ...
    pub fn main() -> io::Result<()> {
        if std::env::args().any(|a| a == "-h" || a == "--help") {
            eprintln!(
                "Usage: cargo run -p ratatui-components --features mdstream,syntect --example mdstream"
            );
            return Ok(());
        }
        ...
        let mut stream_view = MarkdownStreamView::with_options(
            ratatui_components::markdown::streaming::MdStreamOptions::default(),
            MarkdownViewOptions {
                padding_left: 1,
                padding_right: 1,
                show_code_line_numbers: true,
