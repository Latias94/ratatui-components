...
impl DiffView {
    ...
    pub fn render_ref(&mut self, area: Rect, buf: &mut Buffer, theme: &Theme) {
        ...
        let highlighted =
            if self.options.highlight_hunks && self.highlighter.is_some() && content_w > 0 {
        ...
        for row in 0..content_area.height {
            ...
            match line.kind {
                DiffLineKind::Add | DiffLineKind::Del | DiffLineKind::Context => {
                    ...
                    {
                        spans = apply_modifier_to_ranges(spans, ranges, Modifier::REVERSED);
                    }
                ...
                _ => {
                    let visible = render::slice_by_cols(&line.content, self.state.x, content_w);
                    buf.set_stringn(
                        content_area.x + gutter_w,
                        y,
                        visible,
            ...
            let line = match l.kind {
                DiffLineKind::Add | DiffLineKind::Del | DiffLineKind::Context => {
                    ...
                    if self.options.highlight_inline_changes
                        && let Some(ranges) = self.inline_ranges.get(&idx)
                    {
                        rest = apply_modifier_to_ranges(rest, ranges, Modifier::REVERSED);
                    }
                    spans.extend(rest);
                    Line::from(spans)
                }
